...
if os.path.exists(os.path.join(path_models, "sampling_boostCROP.json")):
    # Load sampling from disk
    with open(os.path.join(path_models, "sampling_boostCROP.json"), "r") as json_reader:
    ...
    for i, fold in enumerate(subsets):
        ...
        sampling_dict["cv_" + str(i)] = {"x_train": x_train.tolist(),
                                         ...
                                         "x_val": x_val.tolist(),
                                         "y_val": y_val.tolist()}
    with open(os.path.join(path_models, "sampling_boostCROP.json"), "w") as file:
...
path_arch = os.path.join(path_models, "boostCROP_" + arch)
...
for i, fold in enumerate(subsets):
    ...
    # Train model
    model.train(train_gen, val_gen, epochs=100, iterations=250,
