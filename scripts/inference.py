...
for model_subdir in os.listdir(path_models):
    ...
    for i in range(0, k_fold):
        ...
        if os.path.exists(os.path.join(path_res, "classifier." + arch + "." + \
                                      "cv_" + str(i) + ".inference.simple.csv")) and \
           os.path.exists(os.path.join(path_res, "classifier." + arch + "." + \
...
for model_subdir in os.listdir(path_models):
    ...
    for i in range(0, k_fold):
        ...
        if os.path.exists(os.path.join(path_res, "detector." + arch + "." + \
                                      "cv_" + str(i) + ".inference.simple.csv")) and \
           os.path.exists(os.path.join(path_res, "detector." + arch + "." + \
                                         "cv_" + str(i) + ".inference.augmenting.csv")):
            continue
        # Initialize model
        model = Neural_Network(nclasses, channels=3, architecture=arch,
...
for model_subdir in os.listdir(path_models):
    ...
    for i in range(0, 3):
        ...
        # Run Inference Augmenting
        preds = predict_augmenting(model, index_list, path_riadd, n_cycles=30,
                                   img_aug=aug, aggregate="mean",
                                   image_format=image_format, batch_size=64,
